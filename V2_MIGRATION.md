...
- [x] FastAPI 상태 확인·인증 흐름 자동 테스트 추가
...
- 임시 로컬 환경에서 `pytest`: 4개 테스트 통과
- 라이선스 발급 → 로그인 → 장치 바인딩 세션 확인 흐름 통과
...
라이선스 발급·검증과 기본 세션은 새 서버로 이전됐지만 라이선스 취소 관리, 일회용 챌린지, 요청 서명, 강제 장치 바인딩, 파일 암호화·복호화는 아직 연결되지 않았습니다. Tauri 클라이언트도 새 인증 API를 아직 호출하지 않습니다.
