...
- [x] 명시적 서버 로그아웃과 현재 세션 즉시 폐기
- [x] 로그아웃 세션의 미사용 챌린지 삭제와 감사 로그 기록
- [x] 로그아웃 실패 시에도 로컬 세션 정리
...
1. 데스크톱이 현재 Bearer 세션 토큰과 장치 ID로 `POST /api/v2/auth/logout`을 호출합니다.
2. 서버가 세션과 장치 바인딩을 다시 검증합니다.
3. 현재 세션 행의 `revoked_at`을 즉시 설정합니다.
4. 해당 세션에 연결된 사용하지 않은 일회용 챌린지를 삭제합니다.
5. 토큰 원문 없이 세션 ID·라이선스 ID·삭제된 챌린지 수를 감사 로그에 기록합니다.
6. 이후 같은 토큰의 세션 확인·챌린지 발급·파일 요청은 모두 거부됩니다.
7. 데스크톱은 서버 응답과 관계없이 `sessionStorage`의 로컬 토큰을 제거합니다.

다른 활성 세션은 로그아웃 대상이 아니므로 유지됩니다. 라이선스 전체 세션 종료는 라이선스 취소 또는 장치 바인딩 초기화 관리 기능이 담당합니다.
